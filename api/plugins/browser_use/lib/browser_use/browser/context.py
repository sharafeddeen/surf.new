...
from ..browser.views import BrowserError, BrowserState, TabInfo, URLNotAllowedError
...
class BrowserContext:
    ...
    async def _check_and_handle_navigation(self, page: Page) -> None:
        ...
        if not self._is_url_allowed(page.url):
            logger.warning(
                f'Navigation to non-allowed URL detected: {page.url}')
            try:
                await self.go_back()
            except Exception as e:
                logger.error(
            ...
            raise URLNotAllowedError(
                f'Navigation to non-allowed URL: {page.url}')
    ...
    async def is_file_uploader(self, element_node: DOMElementNode, max_depth: int = 3, current_depth: int = 0) -> bool:
        ...
        if element_node.tag_name == 'input':
            is_uploader = element_node.attributes.get(
                'type') == 'file' or element_node.attributes.get('accept') is not None
